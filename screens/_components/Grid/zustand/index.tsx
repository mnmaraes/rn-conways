...
export function Grid({width, height}: Props) {
  ...
  const onSizeChanged = useBoardStateStore(state => state.onSizeChanged)
...
function Cell({position}: CellProps) {
  const isAlive = useBoardStateStore(state => state.boardState?.board.has(position) ?? false)

  return <View style={isAlive ? styles.livingCell : styles.deadCell} />
}
